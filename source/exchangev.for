...
!##########################################################################
        ...
        integer :: tag,ta,ly,h1,h2,h3
        integer :: tsend,trecv
        integer :: ni,nj,nk,nic,nif,njc,njf,nkc,nkf
        integer :: nix,njx,nkx,nixc,njxc,nkxc,nixf,njxf,nkxf
        double precision :: tr1,tr2,tr3,tr4,tr5,tr6,tr7,tr8,tr9
        double precision :: sp2,sp1,sm1,sm2
        ...
        do k=no1,nk+no2; do j=no1-1,nj+no2; do i=no1,ni+no2
        tr1=fi(i-1,j,k-1); tr2=fi(i,j,k-1); tr3=fi(i+1,j,k-1)
        tr4=fi(i-1,j,k);   tr5=fi(i,j,k);   tr6=fi(i+1,j,k)
        tr7=fi(i-1,j,k+1); tr8=fi(i,j,k+1); tr9=fi(i+1,j,k+1)

!        jj=2*j-pl; kk=2*k-pl-1; ii=2*i-pl-1; !(i,k)
!        fif(ib,ii,jj,kk)=(25.0d0*tr1+150.0d0*tr2   
!     & -15.0d0*tr3 +150.0d0*tr4+900.0d0*tr5-90.0d0*tr6   
!     & -15.0d0*tr7 -90.0d0*tr8+9.0d0*tr9)/1024.0d0
!        jj=2*j-pl; kk=2*k-pl-1; ii=2*i-pl; !(i+1,k)
!        fif(ib,ii,jj,kk)=(-15.0d0*tr1 +150.0d0*tr2   
!     & +25.0d0*tr3 -90.0d0*tr4+900.0d0*tr5+150.0d0*tr6   
!     & +9.0d0*tr7 -90.0d0*tr8-15.0d0*tr9)/1024.0d0
!        jj=2*j-pl; kk=2*k-pl; ii=2*i-pl-1; !(i,k+1)
!        fif(ib,ii,jj,kk)=(-15.0d0*tr1 -90.0d0*tr2   
!     & +9.0d0*tr3 +150.0d0*tr4+900.0d0*tr5-90.0d0*tr6   
!     & +25.0d0*tr7 +150.0d0*tr8-15.0d0*tr9)/1024.0d0
!        jj=2*j-pl; kk=2*k-pl; ii=2*i-pl; !(i+1,k+1)
!        fif(ib,ii,jj,kk)=(9.0d0*tr1 -90.0d0*tr2   
!     & -15.0d0*tr3 -90.0d0*tr4+900.0d0*tr5+150.0d0*tr6   
...
!!!!
        ...
        tr1=0.5*(fi(i-1,j,k-1)+fi(i-1,j+1,k-1))
        tr2=0.5*(fi(i,j,k-1)+fi(i,j+1,k-1))
        tr3=0.5*(fi(i+1,j,k-1)+fi(i+1,j+1,k-1))
        tr4=0.5*(fi(i-1,j,k)+fi(i-1,j+1,k))
        tr5=0.5*(fi(i,j,k)+fi(i,j+1,k))
        tr6=0.5*(fi(i+1,j,k)+fi(i+1,j+1,k))
        tr7=0.5*(fi(i-1,j,k+1)+fi(i-1,j+1,k+1))
        tr8=0.5*(fi(i,j,k+1)+fi(i,j+1,k+1))
        tr9=0.5*(fi(i+1,j,k+1)+fi(i+1,j+1,k+1))

!        tr1=0.0625*(9.0d0*fi(i-1,j,k-1)+9.0d0*fi(i-1,j+1,k-1)
!     & -fi(i-1,j-1,k-1)-fi(i-1,j+2,k-1))
!        tr2=0.0625*(9.0d0*fi(i,j,k-1)+9.0d0*fi(i,j+1,k-1)
!     & -fi(i,j-1,k-1)-fi(i,j+2,k-1))
!        tr3=0.0625*(9.0d0*fi(i+1,j,k-1)+9.0d0*fi(i+1,j+1,k-1)
!     & -fi(i+1,j-1,k-1)-fi(i+1,j+2,k-1))
!        tr4=0.0625*(9.0d0*fi(i-1,j,k)+9.0d0*fi(i-1,j+1,k)
!     & -fi(i-1,j-1,k)-fi(i-1,j+2,k))
!        tr5=0.0625*(9.0d0*fi(i,j,k)+9.0d0*fi(i,j+1,k)
!     & -fi(i,j-1,k)-fi(i,j+2,k))
!        tr6=0.0625*(9.0d0*fi(i+1,j,k)+9.0d0*fi(i+1,j+1,k)
!     & -fi(i+1,j-1,k)-fi(i+1,j+2,k))
!        tr7=0.0625*(9.0d0*fi(i-1,j,k+1)+9.0d0*fi(i-1,j+1,k+1)
!     & -fi(i-1,j-1,k+1)-fi(i-1,j+2,k+1))
!        tr8=0.0625*(9.0d0*fi(i,j,k+1)+9.0d0*fi(i,j+1,k+1)
!     & -fi(i,j-1,k+1)-fi(i,j+2,k+1))
!        tr9=0.0625*(9.0d0*fi(i+1,j,k+1)+9.0d0*fi(i+1,j+1,k+1)
!     & -fi(i+1,j-1,k+1)-fi(i+1,j+2,k+1))
...
!     & -2.0d0*fi(i+1,j,k-1) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
!     & -2.0d0*fi(i-1,j,k+1) +15.0d0*fi(i,j,k+1) +3.0d0*fi(i+1,j,k+1))
!        jj=2*j-pl+1; kk=2*k-pl-1; ii=2*i-pl-1; !avg(i,k)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & +3.0d0*fi(i-1,j,k-1) +15.0d0*fi(i,j,k-1)   
!     & -2.0d0*fi(i+1,j,k-1) +15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   -15.0d0*fi(i+1,j,k)   
!     & -2.0d0*fi(i-1,j,k+1) -15.0d0*fi(i,j,k+1) +1.0d0*fi(i+1,j,k+1)+
!     & +3.0d0*fi(i-1,j+1,k-1) +15.0d0*fi(i,j+1,k-1)   
!     & -2.0d0*fi(i+1,j+1,k-1) +15.0d0*fi(i-1,j+1,k)
!     & +128.0d0*fi(i,j+1,k)   -15.0d0*fi(i+1,j+1,k)   
!     & -2.0d0*fi(i-1,j+1,k+1) -15.0d0*fi(i,j+1,k+1)
!     & +1.0d0*fi(i+1,j+1,k+1))
!        jj=2*j-pl+1; kk=2*k-pl-1; ii=2*i-pl; !avg(i+1,k)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & -2.0d0*fi(i-1,j,k-1) +15.0d0*fi(i,j,k-1)   
!     & +3.0d0*fi(i+1,j,k-1) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
!     & +1.0d0*fi(i-1,j,k+1) -15.0d0*fi(i,j,k+1) -2.0d0*fi(i+1,j,k+1)+
!     & -2.0d0*fi(i-1,j+1,k-1) +15.0d0*fi(i,j+1,k-1)   
!     & +3.0d0*fi(i+1,j+1,k-1) -15.0d0*fi(i-1,j+1,k)
!     & +128.0d0*fi(i,j+1,k)   +15.0d0*fi(i+1,j+1,k)   
!     & +1.0d0*fi(i-1,j+1,k+1) -15.0d0*fi(i,j+1,k+1)
!     & -2.0d0*fi(i+1,j+1,k+1))
!        jj=2*j-pl+1; kk=2*k-pl; ii=2*i-pl-1; !avg(i,k+1)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & -2.0d0*fi(i-1,j,k-1) -15.0d0*fi(i,j,k-1)   
!     & +1.0d0*fi(i+1,j,k-1) +15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   -15.0d0*fi(i+1,j,k)   
!     & +3.0d0*fi(i-1,j,k+1) +15.0d0*fi(i,j,k+1) -2.0d0*fi(i+1,j,k+1)+
!     & -2.0d0*fi(i-1,j+1,k-1) -15.0d0*fi(i,j+1,k-1)   
!     & +1.0d0*fi(i+1,j+1,k-1) +15.0d0*fi(i-1,j+1,k)
!     & +128.0d0*fi(i,j+1,k)   -15.0d0*fi(i+1,j+1,k)   
!     & +3.0d0*fi(i-1,j+1,k+1) +15.0d0*fi(i,j+1,k+1)
!     & -2.0d0*fi(i+1,j+1,k+1))
!        jj=2*j-pl+1; kk=2*k-pl; ii=2*i-pl; !avg(i+1,k+1)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & +1.0d0*fi(i-1,j,k-1) -15.0d0*fi(i,j,k-1)   
!     & -2.0d0*fi(i+1,j,k-1) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
...
!     & +3.0d0*fi(i+1,j+1,k+1))
        ...
        do k=no1,nkc+no2; do j=no1-1,njc+no2; do i=no1,nic+no2
        jj=2*j-pl; i1=2*i-pl-1; i2=2*i-pl; k1=2*k-pl-1; k2=2*k-pl
        fic(ib,i,j,k)=0.25*(fi(i1,jj,k1)+fi(i1,jj,k2)+
     &                   fi(i2,jj,k1)+fi(i2,jj,k2))

!        h1=i1+2; h2=jj; h3=k1; sp2=(-fi(h1,h2,h3-1)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0)
!        h1=i1+1; h2=jj; h3=k1; sp1=(-fi(h1,h2,h3-1)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0)
!        h1=i1;   h2=jj; h3=k1; sm1=(-fi(h1,h2,h3-1)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0)
!        h1=i1-1; h2=jj; h3=k1; sm2=(-fi(h1,h2,h3-1)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0)
...
!==========================================================================
           ...
           do ib=1,nbp
              ...
              if(dom(ib)%fine_ng) then
                 ...
                 if(LMR.eq.2) then
                    ...
                    ief=nif-pl;   jef=njf-pl-1; kef=nkf-pl
                    if(ly.eq.0) jsf=pl+2
                    if(ly.eq.0) jef=njf-pl-2
                 else
                    isf=pl+1;     jsf=pl+1;     ksf=pl+1
        ...
        if (dom(ib)%jnext.lt.0 .and. dom(ib)%bc_north.ne.5) jef=jef-1
        ...
        if (dom(ib)%jnext.lt.0 .and. dom(ib)%bc_north.ne.5) jec=jec-1
...
!..........................................................................
     ...
     &dom_ad(dom(ib)%iprev),tag,MPI_COMM_WORLD,ierr)
        ...
        do k=no1,nkc+no2; do i=no1,nic+no2
        j=jsc+ly; ijk=(k-1)*nic+i
        jj=2*j-pl; i1=2*i-pl-1; i2=2*i-pl; k1=2*k-pl-1; k2=2*k-pl
        h1=i1+2; h2=jj; h3=k1; sp2=(-fi(h1,h2,h3-1)+9.0*fi(h1,h2,h3)
     &+9.0*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0)
        h1=i1+1; h2=jj; h3=k1; sp1=(-fi(h1,h2,h3-1)+9.0*fi(h1,h2,h3)
...
!=====> previous cor #2
        if (dom(ib)%corprev2.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%corprev2)) then
              ...
              st1=1; st2=1; st3=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%corprev2)) st2=0
              do pl1=st1,pl; do pl2=st2,pl; do pl3=st3,pl
                 ijk=pl1*pll+pl2*(pl+1)+pl3+1
...
!=====> previous cor #3
        if (dom(ib)%corprev3.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%corprev3)) then
              ...
              st1=1; st2=1; st3=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%corprev3)) st2=0
              do pl1=st1,pl; do pl2=st2,pl; do pl3=st3,pl
                 ijk=pl1*pll+pl2*(pl+1)+pl3+1
...
!=====> next cor #1
        if (dom(ib)%cornext1.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%cornext1)) then
              ...
              st1=1; st2=1; st3=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%cornext1)) st2=0
              do pl1=st1,pl; do pl2=st2,pl; do pl3=st3,pl
                 ijk=pl1*pll+pl2*(pl+1)+pl3+1
...
!=====> next cor #4
        if (dom(ib)%cornext4.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%cornext4)) then
              ...
              st1=1; st2=1; st3=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%cornext4)) st2=0
              do pl1=st1,pl; do pl2=st2,pl; do pl3=st3,pl
                 ijk=pl1*pll+pl2*(pl+1)+pl3+1
...
!=====> previous edge #2
        if (dom(ib)%edgprev2.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%edgprev2)) then
              ...
              st1=1; st2=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%edgprev2)) st1=0
...
!=====> previous edge #6
        if (dom(ib)%edgprev6.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%edgprev6)) then
              ...
              st1=1; st2=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%edgprev6)) st2=0
              do pl1=st1,pl; do pl2=st2,pl; do nn=kspr,kepr
                 ijk=(nn-1)*pll+pl1*(pl+1)+pl2+1
...
!=====> next edge #4
        if (dom(ib)%edgnext4.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%edgnext4)) then
              ...
              st1=1; st2=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%edgnext4)) st1=0
              do pl1=st1,pl; do pl2=st2,pl; do nn=ispr,iepr
                 ijk=(nn-1)*pll+pl1*(pl+1)+pl2+1
...
!=====> next edge #5
        if (dom(ib)%edgnext5.ge.0) then
           if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%edgnext5)) then
              ...
              st1=1; st2=1
!              if(LMR.eq.2 .and.
!     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%edgnext5)) st2=0
              do pl1=st1,pl; do pl2=st2,pl; do nn=kspr,kepr
                 ijk=(nn-1)*pll+pl1*(pl+1)+pl2+1
...
!..............................................................................
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%iprev)) then
                 ...
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%iprev)) jspr=pl
!                 if(LMR.eq.2 .and. dom(ib)%edgprev6.ge.0 .and.
!     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%iprev) .and.
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgprev6)) jepr=nj-pl-1
     ...
     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%jprev)) then
                 do k=kspr,kepr; do i=ispr,iepr; ijk=(k-1)*ni+i
                    fi(i,js-1-ly,k)=(
     & dom(ib) %recvb_m2(ijk)+3.0*fi(i,js-ly,k)-fi(i,js+1-ly,k))/3.0
                 end do; end do
                 else
                 do k=kspr,kepr; do i=ispr,iepr; ijk=(k-1)*ni+i
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%kprev)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%kprev)) jspr=pl
!                 if(LMR.eq.2 .and. dom(ib)%edgprev2.ge.0 .and.
!     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%kprev) .and.
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgprev2)) jepr=nj-pl-1
...
!..............................................................................
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%inext)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%inext)) jspr=pl
!                 if(LMR.eq.2 .and. dom(ib)%edgnext5.ge.0 .and.
!     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%inext) .and.
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext5)) jepr=nj-pl-1
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%jnext)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(ly.eq.0 .and.
     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%jnext)) then
                 do k=kspr,kepr; do i=ispr,iepr; ijk=(k-1)*ni+i
                    fi(i,je+1+ly,k)=(
     & dom(ib) %recvb_p2(ijk)+3.0*fi(i,je+ly,k)-fi(i,je+ly-1,k))/3.0
                 end do; end do
                 else
                 do k=kspr,kepr; do i=ispr,iepr; ijk=(k-1)*ni+i
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%knext)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%knext)) jspr=pl
!                 if(LMR.eq.2 .and. dom(ib)%edgnext4.ge.0 .and.
!     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%knext) .and.
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext4)) jepr=nj-pl-1
