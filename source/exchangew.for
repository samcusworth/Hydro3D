...
!##########################################################################
        ...
        do k=no1-1,nk+no2; do j=no1,nj+no2; do i=no1,ni+no2
        tr1=fi(i-1,j-1,k); tr2=fi(i,j-1,k); tr3=fi(i+1,j-1,k)
        tr4=fi(i-1,j,k);   tr5=fi(i,j,k);   tr6=fi(i+1,j,k)
        tr7=fi(i-1,j+1,k); tr8=fi(i,j+1,k); tr9=fi(i+1,j+1,k)

!        kk=2*k-pl; ii=2*i-pl-1; jj=2*j-pl-1; !(i,j)
...
!        kk=2*k-pl; ii=2*i-pl; jj=2*j-pl-1; !(i+1,j)
...
!        kk=2*k-pl; ii=2*i-pl-1; jj=2*j-pl; !(i,j+1)
...
!        kk=2*k-pl; ii=2*i-pl; jj=2*j-pl; !(i+1,j+1)
...
!        tr2=0.0625*(9.0d0*fi(i,j-1,k)+9.0d0*fi(i,j-1,k+1)
!     & -fi(i,j-1,k-1)-fi(i,j-1,k+2))
!        tr3=0.0625*(9.0d0*fi(i+1,j-1,k)+9.0d0*fi(i+1,j-1,k+1)
!     & -fi(i+1,j-1,k-1)-fi(i+1,j-1,k+2))
!        tr4=0.0625*(9.0d0*fi(i-1,j,k)+9.0d0*fi(i-1,j,k+1)
!     & -fi(i-1,j,k-1)-fi(i-1,j,k+2))
!        tr5=0.0625*(9.0d0*fi(i,j,k)+9.0d0*fi(i,j,k+1)
!     & -fi(i,j,k-1)-fi(i,j,k+2))
!        tr6=0.0625*(9.0d0*fi(i+1,j,k)+9.0d0*fi(i+1,j,k+1)
!     & -fi(i+1,j,k-1)-fi(i+1,j,k+2))
!        tr7=0.0625*(9.0d0*fi(i-1,j+1,k)+9.0d0*fi(i-1,j+1,k+1)
!     & -fi(i-1,j+1,k-1)-fi(i-1,j+1,k+2))
!        tr8=0.0625*(9.0d0*fi(i,j+1,k)+9.0d0*fi(i,j+1,k+1)
!     & -fi(i,j+1,k-1)-fi(i,j+1,k+2))
!        tr9=0.0625*(9.0d0*fi(i+1,j+1,k)+9.0d0*fi(i+1,j+1,k+1)
!     & -fi(i+1,j+1,k-1)-fi(i+1,j+1,k+2))

!        kk=2*k-pl+1; ii=2*i-pl-1; jj=2*j-pl-1; !(i,j)
!        fif(ib,ii,jj,kk)=(25.0d0*tr1+150.0d0*tr2   
!     & -15.0d0*tr3 +150.0d0*tr4+900.0d0*tr5-90.0d0*tr6   
!     & -15.0d0*tr7 -90.0d0*tr8+9.0d0*tr9)/1024.0d0
!        kk=2*k-pl+1; ii=2*i-pl; jj=2*j-pl-1; !(i+1,j)
!        fif(ib,ii,jj,kk)=(-15.0d0*tr1 +150.0d0*tr2   
!     & +25.0d0*tr3 -90.0d0*tr4+900.0d0*tr5+150.0d0*tr6   
!     & +9.0d0*tr7 -90.0d0*tr8-15.0d0*tr9)/1024.0d0
!        kk=2*k-pl+1; ii=2*i-pl-1; jj=2*j-pl; !(i,j+1)
!        fif(ib,ii,jj,kk)=(-15.0d0*tr1 -90.0d0*tr2   
!     & +9.0d0*tr3 +150.0d0*tr4+900.0d0*tr5-90.0d0*tr6   
!     & +25.0d0*tr7 +150.0d0*tr8-15.0d0*tr9)/1024.0d0
!        kk=2*k-pl+1; ii=2*i-pl; jj=2*j-pl; !(i+1,j+1)
!        fif(ib,ii,jj,kk)=(9.0d0*tr1 -90.0d0*tr2   
!     & -15.0d0*tr3 -90.0d0*tr4+900.0d0*tr5+150.0d0*tr6   
!     & -15.0d0*tr7 +150.0d0*tr8+25.0d0*tr9)/1024.0d0
...
!     & +3.0d0*fi(i-1,j-1,k) +15.0d0*fi(i,j-1,k)   
!     & -2.0d0*fi(i+1,j-1,k) +15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   -15.0d0*fi(i+1,j,k)   
!     & -2.0d0*fi(i-1,j+1,k) -15.0d0*fi(i,j+1,k) +1.0d0*fi(i+1,j+1,k))
!        kk=2*k-pl; ii=2*i-pl; jj=2*j-pl-1; !(i+1,j)
!        fif(ib,ii,jj,kk)=0.0078125 * (
!     & -2.0d0*fi(i-1,j-1,k) +15.0d0*fi(i,j-1,k)   
!     & +3.0d0*fi(i+1,j-1,k) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
!     & +1.0d0*fi(i-1,j+1,k) -15.0d0*fi(i,j+1,k) -2.0d0*fi(i+1,j+1,k))
!        kk=2*k-pl; ii=2*i-pl-1; jj=2*j-pl; !(i,j+1)
!        fif(ib,ii,jj,kk)=0.0078125 * (
!     & -2.0d0*fi(i-1,j-1,k) -15.0d0*fi(i,j-1,k)   
!     & +1.0d0*fi(i+1,j-1,k) +15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   -15.0d0*fi(i+1,j,k)   
!     & +3.0d0*fi(i-1,j+1,k) +15.0d0*fi(i,j+1,k) -2.0d0*fi(i+1,j+1,k))
!        kk=2*k-pl; ii=2*i-pl; jj=2*j-pl; !(i+1,j+1)
!        fif(ib,ii,jj,kk)=0.0078125 * (
!     & +1.0d0*fi(i-1,j-1,k) -15.0d0*fi(i,j-1,k)   
!     & -2.0d0*fi(i+1,j-1,k) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
!     & -2.0d0*fi(i-1,j+1,k) +15.0d0*fi(i,j+1,k) +3.0d0*fi(i+1,j+1,k))
!        kk=2*k-pl+1; ii=2*i-pl-1; jj=2*j-pl-1; !avg(i,j)
...
!     & -2.0d0*fi(i+1,j-1,k+1) +15.0d0*fi(i-1,j,k+1)
!     & +128.0d0*fi(i,j,k+1)   -15.0d0*fi(i+1,j,k+1)   
!     & -2.0d0*fi(i-1,j+1,k+1) -15.0d0*fi(i,j+1,k+1)
!     & +1.0d0*fi(i+1,j+1,k+1))
!        kk=2*k-pl+1; ii=2*i-pl; jj=2*j-pl-1; !avg(i+1,j)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & -2.0d0*fi(i-1,j-1,k) +15.0d0*fi(i,j-1,k)   
!     & +3.0d0*fi(i+1,j-1,k) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
!     & +1.0d0*fi(i-1,j+1,k) -15.0d0*fi(i,j+1,k) -2.0d0*fi(i+1,j+1,k)+
!     & -2.0d0*fi(i-1,j-1,k+1) +15.0d0*fi(i,j-1,k+1)   
!     & +3.0d0*fi(i+1,j-1,k+1) -15.0d0*fi(i-1,j,k+1)
!     & +128.0d0*fi(i,j,k+1)   +15.0d0*fi(i+1,j,k+1)   
!     & +1.0d0*fi(i-1,j+1,k+1) -15.0d0*fi(i,j+1,k+1)
!     & -2.0d0*fi(i+1,j+1,k+1))
!        kk=2*k-pl+1; ii=2*i-pl-1; jj=2*j-pl; !avg(i,j+1)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & -2.0d0*fi(i-1,j-1,k) -15.0d0*fi(i,j-1,k)   
!     & +1.0d0*fi(i+1,j-1,k) +15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   -15.0d0*fi(i+1,j,k)   
!     & +3.0d0*fi(i-1,j+1,k) +15.0d0*fi(i,j+1,k) -2.0d0*fi(i+1,j+1,k)+
!     & -2.0d0*fi(i-1,j-1,k+1) -15.0d0*fi(i,j-1,k+1)   
!     & +1.0d0*fi(i+1,j-1,k+1) +15.0d0*fi(i-1,j,k+1)
!     & +128.0d0*fi(i,j,k+1)   -15.0d0*fi(i+1,j,k+1)   
!     & +3.0d0*fi(i-1,j+1,k+1) +15.0d0*fi(i,j+1,k+1)
!     & -2.0d0*fi(i+1,j+1,k+1))
!        kk=2*k-pl+1; ii=2*i-pl; jj=2*j-pl; !avg(i+1,j+1)
!        fif(ib,ii,jj,kk)=0.00390625 * (
!     & +1.0d0*fi(i-1,j-1,k) -15.0d0*fi(i,j-1,k)   
!     & -2.0d0*fi(i+1,j-1,k) -15.0d0*fi(i-1,j,k)
!     & +128.0d0*fi(i,j,k)   +15.0d0*fi(i+1,j,k)   
...
!        h1=i1+2; h2=j1; h3=kk; sp2=(-fi(h1,h2-1,h3)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2+1,h3)-fi(h1,h2+2,h3))/(16.0)
!        h1=i1+1; h2=j1; h3=kk; sp1=(-fi(h1,h2-1,h3)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2+1,h3)-fi(h1,h2+2,h3))/(16.0)
!        h1=i1;   h2=j1; h3=kk; sm1=(-fi(h1,h2-1,h3)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2+1,h3)-fi(h1,h2+2,h3))/(16.0)
!        h1=i1-1; h2=j1; h3=kk; sm2=(-fi(h1,h2-1,h3)+9.0*fi(h1,h2,h3)
!     &+9.0*fi(h1,h2+1,h3)-fi(h1,h2+2,h3))/(16.0)
...
!==========================================================================
           ...
           do ib=1,nbp
              ...
              if(dom(ib)%fine_ng) then
                 ...
                 if(LMR.eq.2) then
                    ...
                    ief=nif-pl;   jef=njf-pl;   kef=nkf-pl-1
                    if(ly.eq.0) ksf=pl+2
                    if(ly.eq.0) kef=nkf-pl-2
                 else
                    isf=pl+1;     jsf=pl+1;     ksf=pl+1
        ...
        if (dom(ib)%knext.lt.0 .and. dom(ib)%bc_top.ne.5) kef=kef-1
        ...
        if (dom(ib)%knext.lt.0 .and. dom(ib)%bc_top.ne.5) kec=kec-1
...
!..........................................................................
     ...
     &dom_ad(dom(ib)%jprev),tag,MPI_COMM_WORLD,ierr)
        ...
        do j=no1,njc+no2; do i=no1,nic+no2
        k=ksc+ly; ijk=(i-1)*njc+j
        kk=2*k-pl; i1=2*i-pl-1; i2=2*i-pl; j1=2*j-pl-1; j2=2*j-pl
        h1=i1+2; h2=j1; h3=kk; sp2=(-fi(h1,h2-1,h3)+9.0*fi(h1,h2,h3)
     &+9.0*fi(h1,h2+1,h3)-fi(h1,h2+2,h3))/(16.0)
        h1=i1+1; h2=j1; h3=kk; sp1=(-fi(h1,h2-1,h3)+9.0*fi(h1,h2,h3)
...
!..............................................................................
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%iprev)) then
                 ...
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%iprev)) kspr=pl
!                 if(LMR.eq.2 .and. dom(ib)%edgnext3.ge.0 .and.
...
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext3)) kepr=nk-pl-1
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%jprev)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%jprev)) kspr=pl
!                 if(LMR.eq.2 .and. dom(ib)%edgnext2.ge.0 .and.
...
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext2)) kepr=nk-pl-1
     ...
     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%kprev)) then
                 do i=ispr,iepr; do j=jspr,jepr; ijk=(i-1)*nj+j
                    fi(i,j,ks-1-ly)=(
     & dom(ib) %recvb_m3(ijk)+3.0*fi(i,j,ks-ly)-fi(i,j,ks+1-ly))/3.0
                 end do; end do
                 else
                 do i=ispr,iepr; do j=jspr,jepr; ijk=(i-1)*nj+j
...
!..............................................................................
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%inext)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%inext)) kspr=pl
...
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext1)) kepr=nk-pl-1
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%jnext)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(LMR.eq.2 .and.
     & rdiv(dom_id(ib)).lt.rdiv(dom(ib)%jnext)) kspr=pl
...
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext4)) kepr=nk-pl-1
              ...
              if(rdiv(dom_id(ib)).ne.rdiv(dom(ib)%knext)) then
                 ...
                 iepr=ni-pl;	jepr=nj-pl;	kepr=nk-pl
                 if(ly.eq.0 .and.
     & rdiv(dom_id(ib)).gt.rdiv(dom(ib)%knext)) then
                 do i=ispr,iepr; do j=jspr,jepr; ijk=(i-1)*nj+j
                    fi(i,j,ke+1+ly)=(
     & dom(ib) %recvb_p3(ijk)+3.0*fi(i,j,ke+ly)-fi(i,j,ke+ly-1))/3.0
                 end do; end do
                 else
                 do i=ispr,iepr; do j=jspr,jepr; ijk=(i-1)*nj+j
